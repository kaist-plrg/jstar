...
import kr.ac.kaist.jstar.{ JSTAR, Command, JSTARConfig }
import kr.ac.kaist.jstar.phase.{ PhaseOption, Config }
...
class ArgParser(cmd: Command, jstarConfig: JSTARConfig) extends RegexParsers {
  ...
  addRule(jstarConfig, "", JSTAR.options)
  ...
  def apply(args: List[String]): Unit = {
        ...
        Source.fromFile(fileName)("UTF-8").mkString.parseJson match {
          case (obj: JsObject) => obj.fields.foreach {
            case (phase, value: JsObject) => {
              if (JSTAR.phases.map(_.name).contains(phase))
    ...
    lazy val fileName: Parser[Unit] = str ^^ {
      s => jstarConfig.args = s :: jstarConfig.args
    }
    ...
    jstarConfig.args = jstarConfig.args.reverse
  }
}
